...
4. **Extract the formal requirements.** For each formal requirement the methodology sources impose on a literature-review plan, list it. Use the sources' own terms. Cite the source and the section where the requirement is named. Where the sources disagree or where one source extends another (for example, Levac extending Arksey & O'Malley), record that explicitly.
