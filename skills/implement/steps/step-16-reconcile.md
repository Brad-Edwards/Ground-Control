...
**This step runs in Phase E, AFTER the PR merges (issue #963)** - the merged PR is what makes the diff "shipped," so reconciling here keeps the traceability graph from ever pointing at code that did not land. Now that CI and all reviews are green, the PR has merged, AND every materially-implemented in-scope requirement is ACTIVE (Step 15), reconcile the Ground Control traceability graph against the actual diff. This MUST happen AFTER Step 15 and BEFORE the post-merge final report (Step 17, `phase="post_merge"`).
