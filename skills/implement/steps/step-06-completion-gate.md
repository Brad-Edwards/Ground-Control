...
5. **Vale prose linting (ADR-054).** `make policy` runs Vale on docs touched in the diff after `vale-install`. If Vale reports errors in any diff-touched `.md` file, fix them before proceeding. Vale is installed on first run by `tools/install-vale.sh` and does not block this gate when the install fails (graceful skip).
