...
- **Authentication**: the underlying `gc_test_quality_review` shells out to `claude --print` with auth declared in the launch directory's `.env` - one of `CLAUDE_CODE_USE_VERTEX`, `CLAUDE_CODE_USE_BEDROCK`, `CLAUDE_CONFIG_DIR`, `ANTHROPIC_API_KEY`, or `ANTHROPIC_AUTH_TOKEN`, never inherited from the launcher or read from a user-level file (issue #1562). With none declared the cycle returns `test_quality_review_auth_missing` with `next_action: "provision_review_engine_auth_in_launch_root_env"`: an operator provisioning fault under the hard-external-dependency pause class, not a station failure and not a spent cycle. Escalate with that concrete request and keep the obligation open. See `docs/DEVELOPMENT_WORKFLOW.md` § "Test-quality review engine" for the full mechanism.
