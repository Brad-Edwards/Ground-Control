...
1. **If the work is NOT yet complete**: produce a written plan and post it as an issue comment. Identify which files need to be created or modified, what tests to write, and what approach to take. Update length follows the canonical succinctness rule in `skills/implement/steps/_review-loop-rules.md`.
   ...
   - **Design-time GRC deliverables (GC-GRC-010).** When the Step 3.5 screening record is `security_relevant`, the plan MUST enumerate the change's GRC deliverables as first-class, structured items — threats to model/update, risks to assess, controls to select and implement, stale entities to refresh — passed to `gc_post_implementation_plan` as `grc_deliverables` (not free prose). Coverage is **kind-aware**: every screening **gap surface** must be covered by a `threat`/`risk`/`control` deliverable (`{kind, target, action}`) and every **stale entity** by a `stale_refresh` deliverable. Control selection happens here so controls ship *with* the feature (secure by design), not retrofitted. **No-defer:** in-scope GRC work may not be pushed to a follow-up issue. A **disposition** (`{type: accept|wontfix|not_applicable, authorized_by, rationale}`) is the only relief valve, and it is honored **only when the run is authorized via `override=true` + `override_reason`** quoting the user's authorization (obtained via a clarification prompt) — a caller-supplied `authorized_by` is recorded metadata, not a server-verifiable authorization; GC-GRC-015 will add graph-verified per-entity dispositions. Candidate threats/controls (GC-GRC-007/008) in the screening record are deterministic suggestions surfaced in the refusal `rendered_scaffold`; they are *not* selected/implemented controls until you model/link them. The tool renders the deliverables into the plan comment as the durable `plan → completion` trace the completion gate (GC-GRC-012) reads.
...
2. **Post the plan to the issue thread** via the `gc_post_implementation_plan` MCP tool with:
   ...
   - `grc_deliverables`: the structured design-time GRC deliverables (see the GC-GRC-010 bullet above) — required when the Step 3.5 screening is `security_relevant`.

   The tool refuses unless BOTH a `preflight` AND a `grc_screening` phase marker exist for this issue (screening must precede planning so deliverables can be derived from the screening sets). If you skipped Step 2.5 or Step 3.5, the gate refuses and instructs you to run the missing step first; do not work around the refusal by `gh issue comment` directly. For a `security_relevant` change with missing/incomplete/deferred `grc_deliverables`, the tool refuses with `uncovered[]`/`invalid[]` and a `rendered_scaffold` — enumerate the deliverables it lists (or record authorized dispositions) and retry. If `cfg.workflow.dev_start_gate.enabled` is `true`, the tool also refuses a missing or invalid `## Dev-Start Gate` section and returns `next_action: add_valid_dev_start_gate_to_plan_and_retry`. The tool also writes a `plan` phase marker so downstream tools can confirm planning happened.
