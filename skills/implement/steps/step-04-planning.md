...
2. **Post the plan to the issue thread** via the `gc_post_implementation_plan` MCP tool with:
   ...
   The tool refuses unless a `contract` phase marker exists for this issue. If you skipped Step 3.5, this gate will refuse the plan post and instruct you to run `gc_post_interface_contract` first; do not work around the refusal by `gh issue comment` directly. The tool also writes a `plan` phase marker so downstream tools can confirm planning happened.
