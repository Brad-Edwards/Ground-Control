...
- **Plain-English outcome (issue #1156)**: Step 19 must pass `plain_english_outcome` to `gc_post_final_report`. The tool renders it as the Phase D outcome section before the structured evidence, so the closeout explains what the change means in product/operator terms without weakening CI, SonarCloud, traceability, or GRC gates.
