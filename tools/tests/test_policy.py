...
class AdrGuardTriggerScopingTest(unittest.TestCase):
    ...
    *when* a trigger fires, never what it then requires — and only where a rule opts in.
    ...
    def test_scoped_path_triggers_when_the_guarded_surface_changed(self):
        with mock.patch(
            "tools.policy.checks.changed_lines_for",
    ...
    def test_unreadable_diff_still_triggers(self):
        ...
        """
        with mock.patch("tools.policy.checks.changed_lines_for", return_value=None):
            self.assertTrue(
                _trigger_is_in_scope(
                    "mcp/ground-control/lib.js", self.RULE, base="origin/dev", root=REPO_ROOT
                )
...
class MeasurementCatalogueChecksTest(unittest.TestCase):
    ...
    # ---- ADR-082: a published contract version is never edited in place -------
    ...
    # ---- gate finding schema: bounded facts, never prose ----------------------
    ...
    def _mirror(self, tmp: str, catalogue: dict, config: str | None = None) -> Path:
        ...
        shutil.copytree(
            REPO_ROOT / "mcp/ground-control/lib", root / "mcp/ground-control/lib"
        )
        if config is None:
