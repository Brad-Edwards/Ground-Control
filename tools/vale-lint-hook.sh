...
#
# Lints the staged .md / .markdown files passed by pre-commit. If Vale is
# not installed on this machine, install it via tools/install-vale.sh
# (one-time download, idempotent) and then run. Skipping the lint when
# Vale is missing would let agents commit unlinted prose, which is the
