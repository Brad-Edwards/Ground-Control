...
- Deterministic control identification and mapping over enumerated threats (GC-GRC-008): a built-in,
  ...
  graph-queryable. Ships a read-only `/api/v1/control-identification` API (identify, coverage) plus a
  confirmation write route, and a `gc_control_identification` MCP tool. No LLM is involved in the
  mapping path, so control selection stays auditable against recognized frameworks.
