...
- **Research artifact provenance ledger (ADR-069)**: a run-scoped, append-only
  ...
  idempotent on a run-scoped key and rework-aware (supersession, never in-place
  mutation); self-edges and directed cycles are rejected; creation/mutation are
  audited via Hibernate Envers with a server-set actor.
- **Provenance REST + MCP surface**: `ResearchProvenanceController` under
  ...
  and a bounded backward `nodes/{nodeId}/chain` traversal that answers which
