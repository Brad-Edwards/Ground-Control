...
- **Workflow-run telemetry & economics reporting (issue #859, ADR-061)**: a
  ...
  database-side over bounded windows; reads are project-scoped with an admin-only
  cross-project operator rollup; the closed, redacted field set excludes prompts,
  completions, tokens, keys, and raw reviewer payloads, and a `provenance` seam
