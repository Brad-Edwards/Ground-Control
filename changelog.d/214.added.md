...
- **Scheduled evidence collection (GC-S005, ADR-074)**: evidence collection
  ...
  `connectionEndpoint` is SSRF-guarded at create/update (http/https only; the
  host is resolved and rejected if it maps to loopback, link-local/cloud-metadata,
  private, IPv6 unique-local, wildcard, or multicast space).
