...
**Workflow control surface (GC-O009 / ADR-028, issue #1278):** the product-owned REST + MCP surface to drive `/implement` Temporal workflows, kept deliberately distinct from the ADR-061 `WorkflowRun` telemetry projection (`/api/v1/workflow-runs`) which is reporting/correlation only. `WorkflowExecutionController` (`/api/v1/workflow-executions`) starts an execution, lists/gets executions, and sends the closed operator-signal catalog (`cancel`, `retry-from`, `review-cap disposition`; PR merge is observed from GitHub, never signaled). The thin controller delegates to `WorkflowExecutionService` (`domain/workflowexecution`), the single authorization/audit boundary: it resolves the project via `ProjectService`, proves every read/signal targets that project via the slash-free `WorkflowExecutionId` scheme (`gc-implement-<project>-<issue>`; cross-project ids resolve to `404` without leaking existence), validates the closed workflow-type and signal catalogs, and delegates Temporal work to the `WorkflowControlPort`. The single `TemporalWorkflowControlAdapter` (`infrastructure/temporal/control`) is the only place `WorkflowClient`/Visibility is touched - status reads project from Temporal Visibility plus non-secret Memo correlation (`project`, `issueNumber`, `requirementUids`); domain product enums (`WorkflowType`, `OperatorSignalType`, `RetryPhase`, `Reviewer`, `SignalDisposition`, `WorkflowExecutionStatus`) map 1:1 to the Temporal-history contract enums with an exhaustive drift-guarded mapping. Signal routes are `ROLE_ADMIN` in `ApiPathMatrix` (interim until GC-P024 gate authority); start and reads are authenticated + project-scoped. The surface is gated by `groundcontrol.temporal.control.enabled` (reusing the worker's `WorkflowClient`); when disabled the service reports `503` (`ServiceUnavailableException`) so the app stays bootable. The MCP tool `gc_workflow_execution` mirrors the surface. No mirrored Postgres execution state machine (ADR-028).
