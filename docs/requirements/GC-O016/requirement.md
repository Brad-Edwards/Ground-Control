...
(b) One host-wide admission state. Independent processes, MCP server processes, repositories, and linked worktrees owned by the same user shall coordinate through a single per-user admission state protected by an operating-system advisory lock. The lock shall be held only while that state is read, recovered, scheduled against, and atomically replaced, and never for the duration of a command. The dispatcher shall require no long-running daemon.
