...
(b) the classification used for a given run shall be auditable on the issue thread per ADR-029 (that is, the durable record shall show which composition was applied and on what basis),
...
(d) the override authority shall not bypass user-facing safety gates that exist independent of the risk model (for example, the no-merge constraint in Step 19 of `/implement`), and
