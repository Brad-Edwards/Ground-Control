...
updated_at: 2026-08-02T18:42:39.150237Z
---
...
(B) Quality Gate: Before staging, the agent shall pass a hard completion gate requiring: (1) build success via the configured completion command, (2) CHANGELOG updated if source files changed, and (3) clause-by-clause mapping completed. IMPLEMENTS and TESTS traceability links and the requirement-status transition to ACTIVE happen in Phase E after the PR merges, not in Phase B.

(C) Stage, Commit, Push: The agent shall stage files, run pre-commit hooks in a retry loop (max 5 iterations), and push to a feature branch. Before the first push, the agent shall run a single pre-push Codex review pass against the staged + unstaged diff (gc_codex_review with uncommitted=true), fixing findings locally and re-staging until clean.

(D) Multi-Reviewer Ship Pipeline: The agent shall create a PR, monitor CI, validate the SonarCloud quality gate, run a test-quality review (review-tests skill), fix all findings from every reviewer, re-run CI if changes were made, and present the PR for human review and merge with a pre-merge readiness record (the Phase D terminal signal). The agent shall not merge PRs.

(E) Post-Merge Reconciliation: After the user merges the PR, the agent — re-invoked on the issue — shall transition the requirement to ACTIVE, reconcile IMPLEMENTS/TESTS traceability links against the merged diff, post the reconciled final report, and close the issue. Each of these is gated on the linked PR being merged (merged_at non-null AND state MERGED), so Ground Control state never runs ahead of shipped code; a reviewed-but-abandoned PR leaves the requirement DRAFT and unlinked.
...
- IMPLEMENTS → CODE_FILE `mcp/ground-control/index.js` (MCP server tool registration (gc_codex_review/_verify_finding/_post_implementation_plan))
...
- IMPLEMENTS → CODE_FILE `tools/summarize_implement_telemetry.py` (/implement step-telemetry summarizer (make implement-cost-summary))
...
- IMPLEMENTS → CONFIG `Makefile` (Makefile target: implement-cost-summary (ADR-036 § telemetry))
