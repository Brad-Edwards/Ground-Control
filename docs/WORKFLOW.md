...
6. **Transition to ACTIVE in the delivery diff, before publish** (issue #1541): set `status: ACTIVE` in the requirement file so the transition is reviewed in and merged by the PR. It happens before traceability reconciliation.
7. **Reconcile traceability links** against the diff (after the transition above), also in the delivery diff:
...
Pre-merge readiness (Step 17 `phase="pre_merge"`) names this requirement state as *proposed*, not authoritative. After the user merges, **Phase E is validation-only**: it reads the merged requirement files at the immutable merge revision and verifies each in-scope requirement (UID path, frontmatter id, expected status, required traceability), failing closed before the final report on any mismatch and rendering the observed merged values. Phase E makes no requirement-file edits, never manufactures a placeholder edit, and runs no completion command, policy suite, review, or other implementation verification. Its only gate is `gc_implement_mechanical action="finalize"` (merged-state verification + final report + idempotent close), which runs no `verify` (issues #1541, #1543).
...
- **Durable-record MCP tools.** Three new deterministic tools replace agent-authored long-form comments. `gc_post_decision_record` renders the canonical Step 6.5 decision-record from structured findings; `gc_post_final_report` renders the Step 17 final summary (invoked via `gc_assert_completion`); `gc_render_pr_body` composes a PR body that satisfies `check_pr_body`'s policy gates from structured input (`change_class ∈ {doc-only, source, source+migration}` shapes the integration-tests / changelog-fragment cells). Its evidence envelope is repo-neutral (issue #1199): every attestation names a gate the workflow enforces for all repositories, never a stack-specific check (Java/domain rules, Envers, framework layering) or a configured command string. All three filter sensitive content, post under a structured marker family (`gc:decision-record`, `gc:final-report`), and reject `decision: "defer"` server-side. For `/implement`, `gc_post_final_report` now requires `plain_english_outcome` and renders it as an Outcome section before the structured evidence. `gc_render_pr_body` accepts `lane` and `pre_push_reviews` (issue #1551) so the Ground Control Checks pre-push review attestation matches what the run did: the `/implement` default attests that both reviewers completed, and a `/quickfix` run invoked without `--review` attests that they did not run. The attestation is never optional - only accurate - and `/implement` is refused if it tries to claim its mandatory reviewers were skipped.
- **Merged requirement-state + close gates (#1058/#1156/#1103/#1541).** `gc_assert_completion` takes a `phase` parameter. `phase="post_merge"` (the Phase E, default form) is **merge-gated** - it refuses with `completion_pr_not_merged` unless the linked PR is merged - then **verifies every in-scope requirement at the linked PR's immutable merge revision** (re-deriving scope from the issue; exact UID path, frontmatter id, expected status, required traceability), refusing with `completion_requirement_state_unverified` / `completion_scope_mismatch` before `gc_post_final_report` on any mismatch and rendering the observed merged values. `override` + `override_reason` is the recorded escape hatch. `phase="pre_merge"` is the Phase D terminal: it posts a readiness record carrying a `ready_for_review` marker (no `gc:final-report` marker) that names requirement state as *proposed*, and still enforces CI/Sonar/review/scrub gates. Per issue #1541 (superseding #963) the requirement `DRAFT→ACTIVE` transition (Step 15) and traceability reconciliation (Step 16) are made in the delivery diff **before publish** and merge with the PR; Phase E performs no requirement-file mutation. `gc_close_issue_after_merge` is the canonical close path used by Phase E (Step 20): it verifies the linked PR's `merged_at` non-null AND state `MERGED`, and for an open issue requires a trusted `gc:final-report` marker (proof of merged-state validation, `override` available) before running `gh issue close`; it is idempotent on already-closed issues and performs only resolution, verification, and closure - no next-issue recommendation (ADR-089). For a requirement-backed run the PR body uses a non-closing `Refs #<n>` so GitHub does not auto-close ahead of validation; requirement-free runs keep `Closes #<n>`.
...
**2026-09-05 (issue #633 follow-up).** `gc_integration_manager` was restored after #1506 removed it as dead code - its only caller is `skills/integrate/SKILL.md`, which names tools in prose, so the sweep's JS-caller check found none and left GC-O011 (ACTIVE, MUST) with no entry point. Every action is now bound to the MCP launch checkout and refuses a `repo_path` naming another repository the process can reach; the lane writes, so the binding matters more than on the read-only review lane it follows. `skill-tool-registration-contract.test.js` asserts every `gc_*` name in skill prose resolves to a registered tool, so a future dead-code sweep cannot silently remove a live entry point. GC-O011 clause (a) records the binding.
