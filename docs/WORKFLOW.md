...
4. **Post the interface contract**: `gc_post_interface_contract` records the language-neutral public contract (interfaces, DTOs, API/error shapes, invariants) and writes the `contract` marker before planning or coding.
5. **Plan implementation**: posted as a comment on the GitHub issue thread per ADR-029. `gc_post_implementation_plan` refuses without the `contract` marker. The workflow proceeds directly to TDD without a synchronous user-approval gate; the user owns review at PR merge.
...
8. **Transition to ACTIVE** once implemented and verified; the API enforces `IMPLEMENTS-only-on-ACTIVE`, so transition MUST happen before the link-creation step.
9. **Create traceability links** (after the transition above):
...
- **Traceability + post-merge close gates (#1058).** Two new deterministic tools tighten the issue-thread durable record. `gc_assert_traceability_reconciled` (called in Step 17) re-fetches each in-scope requirement and its IMPLEMENTS / TESTS links from the Ground Control REST API and posts a `traceability_reconciled` phase marker on success; the gate is enforced server-side, not by the agent's claim. `gc_post_final_report` (Step 19) refuses unless that marker exists for the issue, with the `/quickfix` lane exempt and a bounded `override_traceability_gate` + `override_traceability_reason` escape for user-authorized skips. `gc_close_issue_after_merge` is the new canonical close path used by Phase E (Step 20): it verifies the linked PR's `merged_at` non-null AND state `MERGED` before running `gh issue close`, and is idempotent on already-closed issues. The PR-body `Closes #<n>` keyword stays as the GitHub UI cross-link but is no longer load-bearing for the close.
