...
- **Traceability + post-merge close gates (#1058/#1156/#1103).** `gc_assert_completion` (Step 17) sequences `gc_assert_traceability_reconciled`, `gc_assert_grc_reconciled`, and `gc_post_final_report` in one deterministic call. The traceability and GRC reconciliation assertions post phase markers (`traceability_reconciled`, `grc_reconciled`) server-side; `gc_post_final_report` refuses without both markers. `gc_close_issue_after_merge` is the canonical close path used by Phase E (Step 20): it verifies the linked PR's `merged_at` non-null AND state `MERGED` before running `gh issue close`, is idempotent on already-closed issues, and returns a best-effort `next_issue_recommendation` or an explicit no-recommendation/failure reason. The PR-body `Closes #<n>` keyword stays as the GitHub UI cross-link but is no longer load-bearing for the close.
