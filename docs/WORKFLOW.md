...
- The current repository's Ground Control context (project id, workflow commands, remote-quality settings, plan rules) lives in `.ground-control.yaml` at the repo root, with larger rule files under `.gc/`. `AGENTS.md` carries a brief pointer to this config so agents know where to look.
...
3. **Checkout feature branch** via `gh issue develop --name <issue-number>-<short-slug>`. The `--name` argument is mandatory (skipping it lets `gh` auto-derive a slug from the full issue title, which produces 100+ character branch names that break terminal display, copy-paste, CI breadcrumbs, and downstream shell quoting). Total branch name ≤ 50 characters, ASCII-only. **Then validate the actual checked-out branch against the same rule**. `gh` reuses an existing branch if the issue was already picked up, so a previous pickup that ran before this rule existed hands the agent a non-compliant branch. The post-check compares against the *remote* base (`origin/<base>` after a fetch; local base can be stale) and renames in place only when no commits or PR exist; otherwise the agent applies the in-progress signal first (so a paused picked-up issue is still visibly flagged) then stops and escalates to the user. The post-check is the dispositive enforcement (the `--name` flag only governs first-time pickups). See `skills/implement/SKILL.md` Step 1 sub-step 11 for the slug-derivation rule, the validation predicate, and worked examples. Then **flag the issue in-progress**: apply an `in-progress` label (created on demand if the repo lacks it) and post a pickup comment on the thread (driver, branch, timestamp) so a maintainer scanning the issue list, or another agent, can see at a glance that the issue is in flight. The label is removed by Step 18 when the agent finishes its work and hands the PR to the user; the GitHub issue itself closes via `Closes #<issue-number>` in the PR body when the user merges. (The label intentionally stays put if a run escalates without finishing.)
4. **Load implementation context and post the interface contract**: `gc_get_implementation_context` bundles binding ADRs, cross-cutting-concern incumbents, existing IMPLEMENTS artifacts, and related-requirement graph context, then writes `context_loaded`. `gc_post_interface_contract` records the language-neutral public contract (interfaces, DTOs, API/error shapes, invariants) and refuses without `context_loaded` and `preflight`.
5. **Plan implementation**: posted as a comment on the GitHub issue thread per ADR-029. `gc_post_implementation_plan` refuses without `context_loaded` and the `contract` marker. The workflow proceeds directly to TDD without a synchronous user-approval gate; the user owns review at PR merge.
6. **Write code, tests, docs**: clause-by-clause verification against the requirement statement and posted contract. TDD is mandatory except for the narrow documentation-only carve-out documented in `skills/implement/SKILL.md` Step 4.4. `gc_assert_test_red` re-runs the targeted test/contract check and writes `test_red`; `gc_assert_impl_green` re-runs the targeted green check and writes `impl_green`.
7. **Run completion gates**: `gc_run_gates` refuses `gates_green` without fresh `impl_green` and runs the ADR-057 assurance classifier. Classified L1/L2 surfaces require the posted contract plus pack-detected contract/test artifacts; L2 also requires property evidence.
8. **Run remote quality**: `gc_watch_required_statuses` verifies required remote statuses plus provider-quality substance server-side before writing `remote_gates_green`. SonarCloud is one provider adapter; a green PR checkmark alone is not enough.
9. **Transition and reconcile traceability**: Step 15 transitions implemented requirements to ACTIVE, calls `gc_reconcile_traceability` to derive the live-diff worklist, applies IMPLEMENTS/TESTS/DOCUMENTS link changes, then calls `gc_assert_traceability_reconciled`. The marker is bound to the live diff hash and `gc_post_final_report` refuses when it is missing or stale.
...
2. **Pre-push Codex review** (Step 6.5, configurable via `workflow.codex_review.pre_push_cap` in `.ground-control.yaml`, ADR-031 effective minimum cap 2). Production-readiness review (`gc_codex_review`, correctness + security + architecture lenses) runs against the staged + unstaged diff *before* the first push, so each fix iteration is local (~5 min) instead of a CI/remote-quality roundtrip (10-15 min). Every successful cycle posts a verbatim findings record to the resolved issue thread (durable per ADR-029) plus inline PR review comments when a PR exists. Each lens returns a verdict envelope; the deterministic MCP dispatcher computes `advance_to_next_phase`, `fix_findings_and_reinvoke`, `post_structured_decision_aid_and_escalate`, or `record_terminal_escalation`. Cycle 1 never collapses into escalation because findings exist.
3. **Pre-push test-quality review** (Step 6.6, configurable via `workflow.test_quality_review.pre_push_cap`, ADR-031 effective minimum cap 2). Moved pre-push by issue #906 from the former post-PR Step 13. `gc_test_quality_review` (MCP tool) is the test-strength lens: it catches assertion-free tests, mock-only assertions, integration-as-unit tests, and tests that can't detect regressions, and consumes `gc_run_gates` mutation / diff-coverage evidence when the manifest supplies it. The tool shells out to `claude --print --model claude-sonnet-4-6` with the canonical rubric, parses structured JSON findings, posts the durable record + cycle marker to the issue thread, and returns a `{findings, cycle, configured_cap, cap, next_action, dispatcher, ...}` envelope. The parent /implement workflow reads dispatcher-computed `next_action` as a directive. Per #884 v2 this replaces the prior `Skill("review-tests")` boundary, which produced prose findings the parent kept echoing back to the user instead of fixing in-turn. Cycle cap is server-side and lane-agnostic. Authentication: the wrapper strips `ANTHROPIC_API_KEY` so claude uses the host's OAuth session. Full mechanism in `architecture/notes/test-quality-review-engine.md`.
4. **CI + remote quality** (Steps 10 / 11): required remote statuses and provider-quality substance (quality gate, issues, ratings, hotspots, coverage, duplication) run after push. SonarCloud is a provider adapter behind `remote_status`, not an engine phase. With both AI-assisted reviewers clean pre-push, the PR opens for human review only after every mechanical and AI-assisted check has passed; there is no post-push review fix loop (former Step 13 / Step 14 were merged out by #906).

The post-push codex review (former Step 12) was removed by issue #804: the pre-push pass catches everything codex would normally flag, and merge-commit drift is the responsibility of CI (compile/tests/integration) and provider-neutral remote-quality gates, not a duplicate codex run. The post-push tool entrypoint (`gc_codex_review` with a `pr_number`) remains as defense-in-depth for direct callers but the SKILL no longer drives it.
...
The `/implement` workflow has three opt-in cost-side optimizations that ride on top of the same gate contract (one human touchpoint at PR merge, configurable ADR-031 convergence cap with effective minimum 2 and per-repo override via `workflow.codex_review.pre_push_cap`, zero deferral, ADR-021/ADR-029 phase structure).
...
- **Context, traceability, remote quality, and post-merge close gates (#1058/#1075).** `gc_get_implementation_context` writes `context_loaded` before contract/plan posting. `gc_reconcile_traceability` computes the live diff and returns the link worklist; `gc_assert_traceability_reconciled` re-fetches requirements/artifact links and writes a diff-bound marker. `gc_watch_required_statuses` verifies required remote statuses plus provider-quality substance server-side before `remote_gates_green`. `gc_post_final_report` refuses unless traceability is reconciled against the live diff. `gc_close_issue_after_merge` remains the canonical Phase E close path after merge.
...
The gate contract, dispatcher-computed cycle decisions, and durable-record posting are unchanged. See
ADR-036 (amendments) for the job model and `skills/implement/steps/step-02.5
/ 06.5 / 06.6` for the operative loop prose.

**2026-05-26 (issue #989).** A new workflow lane, `/integrate`, prepares maintainer-approved pull requests against the latest base branch of a target repository (label-based discovery, rebase, completion gate, CI/remote-quality watch, force-with-lease push). The lane is prepare-only by default: it does not merge. The GC-O007 single-merge-touchpoint contract is unchanged. See `docs/DEVELOPMENT_WORKFLOW.md § /integrate` and GC-O011.
