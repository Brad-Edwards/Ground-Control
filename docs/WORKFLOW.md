...
4. **GRC screening gate** (Step 3.5, per ADR-057): Before planning, classify the planned change surface against the project's existing threat-model and risk-scenario workspaces. Record one of three verdicts (security_relevant, not_security_relevant, no_baseline) as a durable screening record on the issue thread via `gc_post_grc_screening`. This gate operationalizes the GRC-in-the-loop requirement (GC-O012): prose instructions alone carry no enforcement weight; the gate is enforced at the tool layer.
