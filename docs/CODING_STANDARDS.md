...
- `domain/compliance/state/`: `ComplianceFrameworkIdentifier` (`SOC2`, `SOX`, `ISO_27001`, `NIST_CSF`, `PCI_DSS`), `CoverageLevel` (`FULL`, `PARTIAL`, `COMPENSATING`), and `GapSeverity` (`CRITICAL`, `HIGH`, `MEDIUM`, `LOW`, `NONE`) for GC-I002 / GC-I005 / GC-I007 / GC-L011. Pure value enums on the `ComplianceFrameworkMapping` aggregate (typed framework, per-mapping coverage qualifier, cross-framework gap severity); no transitions or invariants on the enums; L0. The polymorphic source endpoint (`requirementId` XOR `controlId`) is enforced by the migration's `CHECK ck_cfm_endpoint_xor` constraint plus the service-layer `validateExactlyOneSourceEndpoint` guard, not on the enum. The free-form external `frameworkIdentifier` string (for genuine externals not in the seeded enum) is bounded at the request DTO (`@Size(max = 200)`) and sanitized for control characters at the service layer before it lands in `limitations` arrays (log-injection guard).
- `domain/audits/state/AuditLinkTargetType`: `FRAMEWORK` is `@Deprecated` since GC-I002 and is preserved for backward compatibility with audit records authored before the `ComplianceFrameworkMapping` aggregate landed (resolves as an external free-form string). New audit links to a compliance framework mapping must use `COMPLIANCE_FRAMEWORK_MAPPING` (added GC-I002 cycle 2), which resolves as an internal target via `ComplianceFrameworkMappingRepository`. Call sites that still dispatch the legacy `FRAMEWORK` arm must carry `@SuppressWarnings("deprecation")`.
