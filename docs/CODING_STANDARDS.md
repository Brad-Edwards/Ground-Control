...
- `domain/controls/state/`: `ControlFunction`, `ControlStatus`, `ControlLinkTargetType`, `ControlLinkType`, `ControlTestMethodology`, `ControlTestConclusion`, `ControlEffectivenessRating` (ADR-039: pure value enums for GC-I012/GC-I013—no transitions, no invariants). Per GC-I017 also adds `FairCamControlDomain` (`LOSS_EVENT_CONTROL`, `VARIANCE_MANAGEMENT_CONTROL`, `DECISION_SUPPORT_CONTROL`); FAIR-CAM domain attribution on `ControlEffectivenessAssessment`; pure value enum, no transitions or invariants; reported beside (never absorbing) `ControlEffectivenessRating` so the three FAIR-CAM dimensions stay independent; L0.
- `domain/riskscenarios/state/`—risk scenario link and status enums; per GC-T004 / C6 adds `ActionItemStatus` (`PLANNED`, `IN_PROGRESS`, `BLOCKED`, `DONE`, `CANCELED`) as the typed status tag on `TreatmentPlan` action items. Distinct from `TreatmentPlanStatus` (item `DONE` ≠ plan `COMPLETED`); no transitions or invariants on the enum, L0. Per GC-T012 also adds `NormalizedConcept` and `CrosswalkVocabularySurface` (`INPUT_SCHEMA`, `OUTPUT_SCHEMA`, `TREATMENT_STRATEGY_VOCABULARY`); pure value enums classifying `CrosswalkEntry` rows on `MethodologyProfile`; no transitions or invariants on the enums; semantic validation lives in `MethodologyProfileService.validateCrosswalkEntries` (duplicate-tuple, surface-presence, field-path resolution, conversion-rule requires scale/units), not on the enum; L0. Per GC-T016 `NormalizedConcept` grows with `PRIMARY_LOSS_MAGNITUDE` and `SECONDARY_LOSS_MAGNITUDE` so FAIR can discriminate the primary vs. secondary loss line; pure additive value-enum growth, L0. Per GC-T016 adds `FairLossForm` (the FAIR-MAM canonical nine-loss taxonomy: `PRODUCTIVITY`, `RESPONSE`, `REPLACEMENT`, `COMPETITIVE_ADVANTAGE`, `FINES_AND_JUDGMENTS`, `REPUTATION`, `CUSTOMER_COMPENSATION`, `NOTIFICATION_AND_CREDIT_MONITORING`, `BUSINESS_INTERRUPTION`) and `FairStakeholderKind` (`ORGANIZATION`, `CUSTOMERS`, `REGULATORS`, `EMPLOYEES`, `INVESTORS`, `PARTNERS`, `PUBLIC`) carrying the FAIR analytics boundary vocabulary; pure value enums, no transitions or invariants; L0.
...
- `domain/riskscenarios/state/` (GC-T005 / T006 / T007 lifecycle additions): `AppetiteToleranceKind` (`QUALITATIVE`, `MONETARY_RANGE`, `LOSS_EVENT_FREQUENCY`, `EXCEEDANCE_PROBORTH`, `COMPOSITE`)—pure value discriminator on `RiskAppetiteTolerance`; no transitions; L0. `KriThresholdBand` (`GREEN`, `YELLOW`, `RED` with an `isBreach()` predicate)—pure value enum classifying KRI measurements; no transitions; L0. `CampaignPhase` (`PLANNING → IDENTIFICATION → ANALYSIS → EVALUATION → TREATMENT → CLOSED`, plus any-phase → `CLOSED` cancellation)—**L2** state machine with a transition table and a `methodologyLocked()` predicate; tested by `RiskAssessmentCampaignTest` and enforced at `RiskAssessmentCampaign.advanceTo`. `ReassessmentTriggerCategory` extended with the GC-T015 NIST §3.4 monitoring categories (`THREAT_CHANGED`, `VULNERABILITY_CHANGED`, `PREDISPOSING_CONDITION_CHANGED`, `OBSERVATION_CHANGED`, `TOPOLOGY_CHANGED`, `ENVIRONMENT_CHANGED`, `KRI_BREACH`) and `ReassessmentTriggerTargetType` extended with the new lifecycle aggregates (`RISK_APPETITE_PROFILE`, `RISK_ASSESSMENT_CAMPAIGN`, `KEY_RISK_INDICATOR`, `OBSERVATION`, `THREAT_MODEL`)—pure value enums; L0.
...
- `domain/evidence/state/`: `EvidenceType` (semantic role tag: `OBSERVATION_SUMMARY`, `CONTROL_TEST_SUMMARY`, `ASSURANCE_CONCLUSION`, `VERIFICATION_SUMMARY`, `ATTESTATION`, `MIXED`), `EvidenceSourceKind` (`OBSERVATION`, `CONTROL_TEST`, `CONTROL_EFFECTIVENESS_ASSESSMENT`, `VERIFICATION_RESULT`, `RISK_ASSESSMENT_RESULT`, `FINDING`, `ATTESTATION`, `EXTERNAL`, plus GC-I003 external kinds `CI_PIPELINE_RESULT` and `SECURITY_SCAN_RESULT` whose opaque identifiers are length-capped and never dereferenced server-side, with an `isInternal()` predicate). Pure value enums for GC-M016 / GC-I003 / ADR-045—no transitions and no invariants; append-only enforcement on the parent `EvidenceArtifact` lives in `EvidenceArtifactService.supersede`, not on the enum (L0).
- `domain/compliance/state/`: `ComplianceDriftCategory` (`CONTROL_STATE_CHANGED`, `EVIDENCE_EXPIRED`, `CODE_CHANGE_IMPACT`, `RESOLUTION`) and `ComplianceDriftSeverity` (`INFO`, `WARN`, `SEVERE`). Pure value enums for GC-I004 / ADR-045 §8—no transitions and no invariants; append-only enforcement on `ComplianceDriftEvent` lives in `ComplianceDriftDetectorService`, not on the enum (L0).
