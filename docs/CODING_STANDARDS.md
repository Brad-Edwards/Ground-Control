...
- `domain/audits/state/AuditLinkTargetType.FRAMEWORK` is `@Deprecated` since GC-I002 in favor of the promoted `ComplianceFrameworkMapping` aggregate. The enum value is preserved for backward compatibility with audit links authored before the aggregate landed; new code should not introduce FRAMEWORK link targets.
