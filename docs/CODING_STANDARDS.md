...
- `domain/riskappetite/state/`: `RiskAppetiteProfileStatus` (`DRAFT`, `ACTIVE`, `RETIRED`; pure value enum for GC-T005 tagging a `RiskAppetiteProfile` version lifecycle; mirrors `MethodologyProfileStatus`. No transitions or invariants on the enum - uniqueness, non-overlapping `ACTIVE` effective windows, and the tolerance-threshold XOR/scale/probability rules live in `RiskAppetiteProfileService`; L0)
...
- `domain/packregistry/state/`: `PackType` (now includes `THREAT_RULE_PACK` constant for GC-GRC-007 threat rule packs; still L0 - no transition logic or invariants), `CatalogStatus`, `TrustOutcome`, `InstallOutcome`, `TrustPolicyField`, `TrustPolicyRuleOperator` enums
...
- `domain/dataclassification/state/`: GC-GRC-006 data classification lattice - `DataClassificationSource` (`DEFAULT` / `CUSTOM`; provenance of the active lattice) and `DataClassificationFindingReason` (`LABEL_FLOW_NOT_PERMITTED` plus the limitation reasons `MISSING_SOURCE_LABEL` / `MISSING_SINK_LABEL` / `UNKNOWN_SOURCE_LABEL` / `UNKNOWN_SINK_LABEL` / `DANGLING_FLOW_ENDPOINT`, each carrying a boolean `isViolation()` flag). Pure value enums with no transitions or invariants; the lattice-soundness rules (antisymmetry, closure, no dangling edges) live in `DataClassificationLatticeFactory`, not on the enums; L0.
- `domain/threatenumeration/state/`: GC-GRC-007 deterministic threat enumeration - `ThreatRuleCategory` (7 semantic groupings for threat rules: `STRIDE_BASELINE`, `DEPLOYMENT_PIPELINE`, `AUTHN_AUTHZ`, `SECRET_HANDLING`, `UNTRUSTED_INPUT`, `DATA_EGRESS`, `CRYPTO`), `ThreatRuleMatchPredicate` (closed predicate vocabulary the deterministic engine evaluates against element views: `ALWAYS`, `CROSSES_TRUST_BOUNDARY`, `SOURCE_IS_EXTERNAL`, `TARGET_IS_EXTERNAL`, `HAS_DATA_CLASSIFICATION`, `HAS_TRUST_BOUNDARY`, `HAS_METADATA_TAG`), and `ThreatEnumerationLimitationReason` (`NO_RULE_PACK_RESOLVED`, `NO_SNAPSHOT`, `UNKNOWN_ELEMENT_KIND`, `MISSING_STABLE_KEY`, `DANGLING_FLOW_ENDPOINT`). All three are pure value enums with no transitions or invariants; the enumeration engine itself is pure-deterministic domain logic that evaluates predicates over element views (no JML contracts needed at L0). L0.
- `domain/controlidentification/state/`: GC-GRC-008 deterministic control identification - `ControlCandidateSource` (`CONTROL_PACK` / `PROJECT_CONTROL`; provenance of a candidate control) and `ControlIdentificationGapReason` (`NO_MATCHING_CONTROL` / `NO_CONTROLS_AVAILABLE`; why a rule fired with no candidate). Pure value enums with no transitions or invariants; the mapping engine (`ControlIdentificationService`) is pure-deterministic domain logic and the confirmation orchestrator reuses the existing `RiskControlMapping` / `ThreatModelLink` services, so no JML contracts apply at L0. L0.
- `domain/grcassessment/state/`: GC-GRC-016 on-demand GRC assessment lane - `GrcAssessmentMode` (`MODEL`, `REASSESS`, `RE_SCREEN`), `GrcAssessmentScopeType`, `GrcAssessmentReviewPolicy`, `GrcAssessmentReviewDecision`, and `GrcAssessmentRunState` are typed value/state tags on the durable run record. The review and graph-commit lifecycle is enforced by `GrcAssessmentRunService` and persisted counters/effects, not by enum transition methods; no enum carries a transition table or invariant. L0.
...
- `domain/evidence/campaign/state/`: `EvidenceCampaignFrequency` (`DAILY`, `WEEKLY`, `MONTHLY`, `QUARTERLY` with a pure `advance(Instant)` calendar-step helper using UTC `ZonedDateTime` arithmetic), `EvidenceCampaignStatus` (`ACTIVE`, `PAUSED`; toggled by `EvidenceCampaignService.pause`/`resume`), `EvidenceCampaignRunStatus` (`PENDING`, `RUNNING`, `COMPLETED`, `PARTIAL`, `FAILED`). Pure value enums for GC-S005 / ADR-074 - no transition table or invariant on the enums; the run lifecycle is set imperatively in `EvidenceCampaignService.executeCampaign` and the scheduling cursor advance lives in `EvidenceCampaignService.runDueCampaigns`, not on the enums; classification L0.
...
- `domain/derivation/state/`: `DerivationScopeMode`, `SystemModelFactKind`, and `CaptureLimitReason` are pure value enums for GC-GRC-001 derivation scopes, normalized fact classes, and capture-limit metadata. Service validation owns path, commit, language, surface, and provenance constraints. No state transitions or invariants live on the enums; classification L0.
