...
- `domain/backlog/state/`: `BacklogItemStatus` (`CANDIDATE → READY → IN_PROGRESS → DONE` plus `ARCHIVED`; `READY` rejects entry unless all four CoD components are present); per ADR-056 / GC-W003. State machine with `validTargets` and `canTransitionTo`; lifecycle invariants live on the `BacklogItem.transitionStatus` method (L0).
- `domain/interchange/state/`: `InterchangeEntityKind` (`OPERATIONAL_ASSET`, `RISK_SCENARIO`, `CONTROL`, `FINDING`, `EVIDENCE_ARTIFACT`)—pure value discriminator that scopes the `(project, entity_kind, external_uid)` uniqueness on `GrcInterchangeProvenance`; no transitions, no invariants; L0—per GC-P012.
- `domain/requirements/state/ArtifactType`: gains `DECISION_RECORD` per GC-W011 / ADR-057 so the existing `TraceabilityLink` substrate can target the new decision-analysis-record aggregate. Still a pure value tag enum (L0).
