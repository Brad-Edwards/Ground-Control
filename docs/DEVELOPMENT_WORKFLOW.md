...
- **Step 3.5 (GRC screening gate)** runs between codebase assessment (Step 3) and planning (Step 4). As of GC-GRC-009 the gate is **derivation-backed (schema `gc.implement.grc-screening/v2`)**: `gc_post_grc_screening` no longer accepts an agent-asserted verdict. It reads the diff, the existing GRC `targetType=CODE` link graph, and the latest/pinned derivation run + architecture-model snapshot, and computes `impact_set` (existing entities the change touches), `gap_set` (touched security-relevant surfaces with no coverage, including `no_derivation_coverage` for the empty/absent-baseline case), and `stale_set` (ACTIVE linked entities whose code changed), attaching deterministic GC-GRC-007/008 candidates when a snapshot exists. **There is no passing `no_baseline` verdict**: a missing baseline creates a scoped `gap_set` (recorded with a capture limit) that must be modeled, controlled, or dispositioned before completion. The `derived_verdict` (`security_relevant` / `not_security_relevant`) is computed. The record uses marker family `<!-- gc:grc-screening -->`, is reproducible from its recorded provenance, and the gate is enforced at the tool layer, not by prose instruction. Historical/in-flight v1 records (`gc.implement.grc-screening/v1`, agent-asserted verdicts) still render and reconcile via the v1 path. See ADR-057 (v1 record + v2 amendment) and ADR-058 (continuous GRC program).
- **Step 4 (design-time GRC deliverables gate)** realizes GC-GRC-010. `gc_post_implementation_plan` requires both a `preflight` and a `grc_screening` marker (screening must precede planning), and when the Step 3.5 `derived_verdict` is `security_relevant` it requires structured `grc_deliverables`: `{kind: threat|risk|control|stale_refresh, target, action}` items, or an authorized `disposition {type: accept|wontfix|not_applicable, authorized_by, rationale}`. Coverage is kind-aware: a `gap_set` surface is covered by a `threat`/`risk`/`control` deliverable and a `stale_set` entity by a `stale_refresh` deliverable. **In-scope GRC work cannot be deferred to a follow-up issue.** A disposition is the single relief valve and is honored only when the run is authorized via `override=true` + `override_reason` quoting the user's authorization (a caller-supplied `authorized_by` is recorded metadata, not a server-verifiable authorization; GC-GRC-015 adds graph-verified per-entity dispositions). On refusal the envelope returns `uncovered[]`/`invalid[]` plus a `rendered_scaffold`; deliverables are modeled by kind (not scraped from prose) and rendered into the plan comment as an authoritative `<!-- gc:grc-deliverables-data -->` machine block: the durable `plan → completion` trace the completion gate (GC-GRC-012) reads. Enforced at the tool layer, not by prose. See ADR-058 §5.
