...
**Async execution (issues #937 and #1473).** Codex review, architecture preflight, test-quality review, repository completion/publish commands, and CI/Sonar watchers can all run longer than one MCP request. Their opt-in `async` mode starts work in one bounded in-process registry and returns `{ok, status: "running", job_id}`; `gc_codex_job` polls until `status: "done"` carries the originating tool's unchanged result under `result`. Review/preflight callers remain cancellable because their child receives an `AbortSignal`. Mechanical `verify`, `publish`, and `monitor` require a bounded `idempotency_key`, reuse an identical logical attempt, reject key conflicts, and enforce single flight for checkout-bound verification/mutation; they return `job_not_cancellable` instead of falsely claiming abort support. Their canonical workflow steps drive the start-then-poll loop, while omitted `async` preserves synchronous direct-caller behavior and the short mechanical actions reject background mode. The registry is bounded, never evicts running work, and retains terminal results for 30 minutes; it is operational request-durable state, not restart-durable workflow evidence. Client-side timeout settings remain headroom, not the correctness mechanism. Full design is in ADR-036.
