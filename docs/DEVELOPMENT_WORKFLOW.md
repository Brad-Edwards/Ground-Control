...
- **The completion gate (step 8) evaluates the project's quality gates** server-side via `gc_assert_quality_gates` (issue #1101) and blocks the run on any failing gate. The failure envelope lists each failing gate as `{name, metric_type, threshold, actual}` so the metric to fix is obvious from the error alone. The enforced metric types are `COVERAGE` (over IMPLEMENTS / TESTS / DOCUMENTS link coverage for the project's ACTIVE requirements), `ORPHAN_COUNT`, and `COMPLETENESS`. The gates themselves are declared in `tools/ground_control/policy.json` and synced to the live instance with `make sync-ground-control-policy`; the same evaluation runs in CI via `make policy-live`. The backend (`QualityGateService.evaluate`) owns all gate math; the tool only shapes the pass/fail envelope at the workflow boundary, returning only the failing gates.
...
| Traceability + post-merge close gates (#1058) | `gc_assert_traceability_reconciled` (Step 17) re-fetches each in-scope requirement and its IMPLEMENTS / TESTS links from the Ground Control REST API and posts a `traceability_reconciled` phase marker on success; `gc_post_final_report` (Step 19) refuses to publish without that marker; `gc_close_issue_after_merge` (Step 20 / Phase E) verifies the linked PR's `merged_at` non-null AND state `MERGED` before closing the issue, idempotent on already-closed issues. The /quickfix lane is requirement-free and exempt from the traceability gate; a bounded `override_traceability_gate=true` + `override_traceability_reason` escape lets the user authorize a skip with a quoted rationale. | Always on for `/implement`; `lane: "quickfix"` opts out of the traceability prerequisite |
| GRC reconciliation gate (#1100) | `gc_assert_grc_reconciled` (Step 17, after `gc_assert_traceability_reconciled`) reads the GRC screening record written by `gc_post_grc_screening` (Step 3.5) from the issue thread and verifies it server-side: for `security_relevant` verdicts it resolves each entity ref (threat model, risk scenario, control) via the REST API and confirms each claimed `CODE` link exists on the owner entity; for `not_security_relevant` / `no_baseline` verdicts the gate passes immediately. On success it posts the `grc_reconciled` phase marker. `gc_post_final_report` (Step 19) now requires **both** `traceability_reconciled` **and** `grc_reconciled` markers before publishing. The /quickfix lane is exempt from the gate. The assertion has no per-tool override; a user-authorized skip of the completion-gate prerequisite is taken at `gc_post_final_report`'s phase override, which bypasses both markers together. | Always on for `/implement`; `lane: "quickfix"` opts out; a user-authorized skip is taken at the final-report phase override |
