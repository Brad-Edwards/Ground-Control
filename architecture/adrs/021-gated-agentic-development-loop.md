...
4. **Phase D: Multi-Reviewer Ship Pipeline**: Create PR, monitor required remote status checks, ~~Codex cross-model review~~ *(post-push codex review removed by issue #804; the canonical codex pass is the pre-push Step 6.5 with correctness, security, and architecture lenses, a configurable ADR-031 convergence cap, and an effective minimum cap of 2; per-repo override via `.ground-control.yaml::workflow.codex_review.pre_push_cap`, configured bounds `[1, 10]`; the override-per-cycle escape via `override_cap=true` + `override_reason` applies after a structured decision aid)*, ~~Claude `/review` and `/security-review` skills~~ *(test quality review only; **moved pre-push to SKILL Step 6.6 by issue #906** from the former post-PR Step 13. Implemented as the **MCP tool** `gc_test_quality_review` with a server-side configurable ADR-031 convergence cap (per-repo override via `workflow.test_quality_review.pre_push_cap`, effective minimum 2). The v1 fix shipped under `Skill("review-tests")` and a workflow-discipline cap; per #884 v2 the boundary moved to an MCP tool because the Skill-tool boundary's autoregressive bias (presenting results to the user instead of fixing them in-turn) defeated the SKILL.md "do not echo, fix in-turn" rule. The MCP tool returns a structured envelope with dispatcher-computed `next_action` that the parent reads as a directive, not a status report. Engine: shells out to `claude --print --model claude-sonnet-4-6` with `--json-schema` validation; strips `ANTHROPIC_API_KEY` from the subprocess env so claude uses the host's OAuth session. Cycle cap enforced server-side via `gc:test-quality-review-cycle` markers on the issue thread; cycle cap+1 requires `override_cap=true` + non-empty `override_reason`. Every cycle posts `gc_post_decision_record(reviewer: "test-quality", findings: [...])`; a clean cycle's empty-findings record is the structured advance-to-Phase-C signal only after the post returns `ok: true`. Full mechanism doc: `architecture/notes/test-quality-review-engine.md`)*, fix all findings, transition requirement to ACTIVE, reconcile traceability links, present for human merge.
...
**2026-05-19 (issue #931), refined by ADR-031 (2026-06-06).** The pre-push
review gate (Step 6.5 codex + Step 6.6 test-quality) now consumes verdict
envelopes (`verdict`, `reviewer_lens`, `findings[]`, `blocking[]`, capped
`notes[]`) rather than a findings-only payload. The gate contract is
unchanged: all blocking lenses must return clean before push, but a clean
cycle now emits `verdict: ship` as a first-class outcome instead of relying on
an empty findings array. The wrapper's `next_action` is computed by the
non-LLM dispatcher, not reviewer prose. See ADR-029 (amendments) and ADR-031
(amendments) for the envelope shape; see `skills/implement/SKILL.md` Step
6.5 / 6.6 for the operative loop prose.
...
ADR-060, ADR-061, and ADR-062 refine GC-O007 without changing the single human
...
impl_green -> gates_green -> reviews_clean -> published ->
remote_gates_green -> traceability_reconciled -> reported -> closed`. Local
advance is blocked by manifest gates, and post-publication advance is blocked
