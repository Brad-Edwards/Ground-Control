...
- If the CLI is Python, we factor `knowledge_ingest.js` and its helpers out to a neutral location (for example, `tools/ground_control/knowledge/`) AND the #525 slice owns the policy carve-out and the language-bridge decision. The refactor is local to this module: nothing else in the codebase imports it.
