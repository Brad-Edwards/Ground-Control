...
| `domain/controlidentification/state/ControlCandidateSource.java` | L0 | Pure value enum; provenance of a GC-GRC-008 candidate control (`CONTROL_PACK`, `PROJECT_CONTROL`); no transitions or invariants |
| `domain/controlidentification/state/ControlIdentificationGapReason.java` | L0 | Pure value enum; why a GC-GRC-008 mapping rule fired with no candidate control (`NO_MATCHING_CONTROL`, `NO_CONTROLS_AVAILABLE`); no transitions or invariants |
| `domain/grcassessment/state/` | L0 | Typed run metadata for GC-GRC-016 (`GrcAssessmentMode`, `GrcAssessmentScopeType`, `GrcAssessmentReviewPolicy`, `GrcAssessmentReviewDecision`, `GrcAssessmentRunState`); review/commit sequencing lives in `GrcAssessmentRunService`, not enum transition methods |
...
- Domain model methods where invalid input causes silent data corruption (for example, status/archivedAt invariant)
...
**OpenJML ESC** (Extended Static Checking) - integrated in Phase 2-B:
...
- Scope: `domain/requirements/state/` and `domain/verification/state/` only (pure enums with no framework annotations). Other `state/` packages (`assets`, `controls`, `riskscenarios`, `riskcontrol`, `plugins`, `controlpacks`, `packregistry`, `threatmodels`, `findings`, `audits`, `derivation`) contain value enums not covered by ESC.
...
- `domain/derivation/state/` (`DerivationScopeMode`, `SystemModelFactKind`, `CaptureLimitReason`) is L0. These are pure value enums for GC-GRC-001 derivation routing, normalized fact typing, and metadata-only capture-limit records. Service validation owns scope, path, language, surface, and provenance constraints; no enum has transition logic or invariants.
- `domain/grcassessment/state/` (`GrcAssessmentMode`, `GrcAssessmentScopeType`, `GrcAssessmentReviewPolicy`, `GrcAssessmentReviewDecision`, `GrcAssessmentRunState`) is L0. These enums tag the durable GC-GRC-016 on-demand assessment run, its partitioning mode, and its review outcome. The only state progression (`READY_FOR_REVIEW` to `COMMITTED` or `REJECTED`) is owned by `GrcAssessmentRunService`, which records bounded graph-effect metadata and rejects invalid command shapes; the enums themselves have no transition table or invariant.
- `domain/evidence/campaign/state/` (`EvidenceCampaignFrequency`, `EvidenceCampaignStatus`, `EvidenceCampaignRunStatus`) is L0 per ADR-074 (GC-S005). `EvidenceCampaignFrequency` (`DAILY`, `WEEKLY`, `MONTHLY`, `QUARTERLY`) is a pure value enum carrying a deterministic `advance(Instant)` calendar-step helper (UTC `ZonedDateTime` arithmetic) - a pure function over an instant, not a state transition. `EvidenceCampaignStatus` (`ACTIVE`, `PAUSED`) is a two-state lifecycle toggled imperatively by `EvidenceCampaignService.pause`/`resume`, and `EvidenceCampaignRunStatus` (`PENDING`, `RUNNING`, `COMPLETED`, `PARTIAL`, `FAILED`) records a run's terminal disposition set imperatively in `EvidenceCampaignService.executeCampaign`. No enum carries a transition table or invariant: the scheduling-cursor advance, the optimistic claim, and the run-status mapping all live in the service, and invalid request values route through Jackson binding into `GlobalExceptionHandler`'s 422 envelope. Scheduled evidence collection is an operational sweep surface; it does not need property-based verification at this assurance level.
...
- `domain/riskcontrol/state/` adds `MappingControlRole` (`PREVENTIVE`, `DETECTIVE`, `CORRECTIVE`, `DETERRENT`, `COMPENSATING`, `RECOVERY`, `DIRECTIVE`) per GC-T003 (ADR-052). Pure value enum used as the typed role tag on `RiskControlMapping`; no state transitions or invariants. Classification: L0 - Jackson binding plus `GlobalExceptionHandler`'s 422 envelope constitute the full verification contract.
- `domain/riskscenarios/state/` adds `ActionItemStatus` (`PLANNED`, `IN_PROGRESS`, `BLOCKED`, `DONE`, `CANCELED`) per GC-T004 / C6 (issue #862). Pure value enum used as the typed status tag on per-action-item value records carried in `TreatmentPlan.actionItems`. Distinct from `TreatmentPlanStatus`, which carries the plan-level lifecycle with its own transition rules - action-item status flips freely as work progresses, and `DONE` at the item level is not the same as `COMPLETED` at the plan level. No transitions, no invariants on the enum itself. Classification: L0 - Jackson binding plus `GlobalExceptionHandler`'s 422 envelope constitute the full verification contract; per-item required-field rejection lives in Bean Validation on the request DTO and a service-layer guard in `TreatmentPlanService.validateActionItems`, not on the enum.
- `domain/riskappetite/state/` adds `RiskAppetiteProfileStatus` (`DRAFT`, `ACTIVE`, `RETIRED`) per GC-T005 (issue #260). Pure value enum tagging the lifecycle of a `RiskAppetiteProfile` version; mirrors `MethodologyProfileStatus` (a registry-entry lifecycle, not a domain-state transition). No transition method or invariant lives on the enum: `RiskAppetiteProfileService` owns the load-bearing invariants (`(project, appetiteKey, version)` uniqueness, non-overlapping effective windows among `ACTIVE` versions, and the tolerance-threshold XOR/scale/probability rules) and surfaces them through `ConflictException`/`DomainValidationException` (no silent corruption), so the enum stays L0. The read-only `RiskAppetiteEvaluationService` derives breach/escalation views without mutating any row.
...
- Gradle up-to-date checking: skips when source files haven't changed (~1 s no-op)
...
- Not every enum under `domain/**/state/` is a state machine. Tag enums that
  ...
  carry NIST SP 800-30 Rev. 1 vocabulary at the API/MCP boundary, with
  `ThreatSourceRelevance` retained as the enum name for the Table E-4
  ...
  threat enumeration engine; `ControlCandidateSource` and
  `ControlIdentificationGapReason` added for GC-GRC-008 / #1121 to tag the
  provenance of a candidate control and the reason a mapping rule produced no
  candidate in the deterministic control-identification engine; and the
  `GrcAssessment*` enums added for GC-GRC-016 / #1129 to tag assessment-run
  mode, scope, review policy/decision, and persisted run state) are L0 data
  classifiers, not L1+ contract
