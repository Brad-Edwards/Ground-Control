...
- Scope: `domain/requirements/state/` and `domain/verification/state/` only (pure enums with no framework annotations). Other `state/` packages (`assets`, `controls`, `riskscenarios`, `riskcontrol`, `plugins`, `controlpacks`, `packregistry`, `threatmodels`, `findings`, `audits`) contain value enums not covered by ESC.
...
- `domain/compliance/state/` adds `ComplianceFrameworkIdentifier` (`SOC2`, `SOX`, `ISO_27001`, `NIST_CSF`, `PCI_DSS`), `CoverageLevel` (`FULL`, `PARTIAL`, `COMPENSATING`), and `GapSeverity` (`CRITICAL`, `HIGH`, `MEDIUM`, `LOW`, `NONE`) per GC-I002 / GC-I005 / GC-I007 / GC-L011. Pure value enums on the `ComplianceFrameworkMapping` aggregate (typed framework, per-mapping coverage qualifier, gap-severity discriminator on the cross-framework analysis result). The polymorphic source endpoint (`requirementId` XOR `controlId`) is enforced by the migration's `CHECK ck_cfm_endpoint_xor` constraint plus the service-layer `validateExactlyOneSourceEndpoint` guard, not by the enums themselves. Classification: L0 across all three—Jackson binding plus `GlobalExceptionHandler`'s 422 envelope constitute the full verification contract. The framework-side string `frameworkIdentifier` (free-form, for genuine externals not in the seeded enum) is bounded at the request DTO (`@Size(max = 200)`) and sanitized for control characters at the service layer to guard against log injection when the value is echoed into `limitations`.
- `domain/audits/state/AuditLinkTargetType`: the `FRAMEWORK` constant is `@Deprecated` since GC-I002 and is preserved for backward compatibility with audit records authored before the `ComplianceFrameworkMapping` aggregate landed (resolves as an external free-form string via `GraphTargetResolverService.validateAuditTarget`). GC-I002 cycle 2 adds `COMPLIANCE_FRAMEWORK_MAPPING` as the typed successor, resolving as an internal target via `ComplianceFrameworkMappingRepository`. Call sites that still dispatch the legacy `FRAMEWORK` arm carry `@SuppressWarnings("deprecation")`. The enum remains L0; the `AuditStatus` lifecycle is unchanged.
