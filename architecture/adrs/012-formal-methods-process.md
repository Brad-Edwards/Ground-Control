...
- Scope: `domain/requirements/state/` and `domain/verification/state/` only (pure enums with no framework annotations). Other `state/` packages (`assets`, `controls`, `riskscenarios`, `riskcontrol`, `plugins`, `packregistry`, `threatmodels`, `findings`, `audits`) contain value enums not covered by ESC.
...
- Not every enum under `domain/**/state/` is a state machine. Tag enums that
  ...
  `AssetLinkTargetType` and `ObservationCategory`) are L0 data
