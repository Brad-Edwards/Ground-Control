...
- `domain/audits/state/AuditLinkTargetType.FRAMEWORK` is marked `@Deprecated` since GC-I002 in favor of the promoted `ComplianceFrameworkMapping` aggregate. The enum value is preserved for backward compatibility with audit links authored before the aggregate landed; the next ADR (post-promotion) carves out the deprecation path, after which a follow-up migration will rewrite extant FRAMEWORK rows to point at the aggregate.
