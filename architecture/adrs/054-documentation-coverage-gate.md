...
**2026-06-06 (issue #1075 release, upgrade, and eval suite).** Extended the gate-pack registry into a release-artifact surface: `workflow/tools/materialize-pack-registry.mjs` now emits `workflow/releases/gc-engine-<version>.tgz`, `workflow/releases/gc-gate-pack-<id>-<version>.tgz`, and a catalog whose enforced checksums cover the release artifacts. `installWorkflowAssets` now verifies and vendors both engine and pack artifacts, tightens `.gc/workflow-lock.json` validation, and supports explicit install versus upgrade mode. Added `workflow/tools/verify-workflow-release.mjs`, `workflow/tools/eval-workflow-engine.mjs`, the versioned eval definitions under `workflow/evals/`, and the `Workflow Platform` CI workflow. The workflow reference documents release artifacts, upgrade behavior, pack self-test matrix behavior, lockfile fields, and eval-suite ownership in `docs/DEVELOPMENT_WORKFLOW.md § Gate Engine Core`; `docs/DOC_STYLE.md` names release artifacts and evals as portable gate-engine documentation surfaces. Signature/provenance enforcement remains documented as TODO metadata per ADR-062; checksum verification and lockfile validation are current behavior. No change to the Vale rule set, the `tools/install-vale.sh` installer, or the `.vale.ini` configuration is required.
