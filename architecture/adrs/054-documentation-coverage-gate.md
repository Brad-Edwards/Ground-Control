...
**2026-05-30 (cluster decision-strategy-aggregates security carve-out).** The `lib.js` change in this commit exports `WSJF_ITERATIONS_MAX`, mirroring the backend's `BacklogItemController.MAX_WSJF_ITERATIONS` cap so MCP-side validation can match the server contract. The `index.js` change extends the comment header documenting the public contract for the WSJF endpoint, the Cost-of-Delay `attributedTo` overwrite, and the decision-analysis `inputs._attributedTo` overwrite. Both are surface annotations for a security fix, not new MCP tool registrations. The documentation lives in `docs/API.md § Backlog Items` and `docs/API.md § Decision Analysis Records`; no change to the Vale rule set, the `.vale.ini` configuration, or `docs/DOC_STYLE.md` itself is required.
...
**2026-05-30 (issues #781 / #789 / #752 backlog + decision + interchange aggregates).** Added `"DECISION_RECORD"` to the `ARTIFACT_TYPES` constant array in `mcp/ground-control/lib.js` to mirror the new `ArtifactType.DECISION_RECORD` Java enum constant (ADR-034 pipeline). Extended the `GC_QUERY_PATH_ALLOWLIST` in `mcp/ground-control/gc-query.js` with `/api/v1/backlog-items` and `/api/v1/decisions`. These are additive MCP-adapter surfaces; documentation lives in `docs/API.md` (new Backlog / Decisions / GRC Interchange sections) and in two new ADRs (ADR-056, ADR-057). No change to the Vale rule set, the `tools/install-vale.sh` installer, the `.vale.ini` configuration, or `docs/DOC_STYLE.md`.
