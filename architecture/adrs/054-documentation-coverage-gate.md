...
**2026-05-30 (issue #1058 follow-up: multi-issue Closes in gc_render_pr_body).** Extended `runRenderPrBody` and `validatePrBodyInput` in `mcp/ground-control/lib.js` with an optional `additionalCloses` array of positive issue numbers, and exposed the field in the `gc_render_pr_body` Zod schema in `mcp/ground-control/index.js` as `additional_closes`. The renderer now emits one `Closes #<n>` line per entry under `## Related Issues` (in input order, after the primary `Closes #<issueNumber>`), so multi-issue clusters no longer need to hand-append Closes lines that would vanish on the next render. Validation rejects non-arrays, non-positive integers, duplicates within the array, and entries that collide with the primary `issueNumber`. The tool documentation lives in the `gc_render_pr_body` tool description string in `mcp/ground-control/index.js`. This is a single MCP-adapter / config-parser surface extension; no change to the Vale rule set, the `tools/install-vale.sh` installer, the `.vale.ini` configuration, or `docs/DOC_STYLE.md`.
