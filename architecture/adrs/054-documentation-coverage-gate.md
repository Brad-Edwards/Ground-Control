...
**2026-05-31 (issue #763 GC-I004 evidence expiry / compliance drift, Sonar cleanup).** The `listEvidenceArtifacts` function in `mcp/ground-control/lib.js` received a JSDoc comment clarifying the `expired`/`expiredOnly` filter semantics; `mcp/ground-control/index.js` received an inline comment update for `gc_evidence` documenting the same filter contract. These are comment-only changes to API-client and tool-registration surfaces already covered by the classifier; no new tool surface, Zod shape, or endpoint was added. `docs/API.md` documentation for the expiry filter was expanded to make the `expired=false` (exclude-expired) and no-param (return-all) behaviors explicit. `docs/DOC_STYLE.md` was updated with a style note on documenting query-parameter aliases. No change to the Vale rule set, `.vale.ini`, the classifier, or any enum contract inventory.

**2026-05-30 (issues #689/#749/#750/#751 GC-Q003/Q011/Q012/Q013 GRC workspace read views).** Added four thin API-client functions to `mcp/ground-control/lib.js` (`getTraceabilityMatrix`, `getControlWorkspace`, `getEvidenceExplorer`, and `analyzePortfolio`) for the new read endpoints `GET /api/v1/requirements/traceability/matrix`, `GET /api/v1/controls/workspace`, `GET /api/v1/evidence-artifacts/explorer`, and `GET /api/v1/analysis/grc/portfolio`. Registered the `gc_traceability_matrix`, `gc_control_workspace`, and `gc_evidence_explorer` tools in `mcp/ground-control/index.js`, and added a `portfolio` kind to the existing `gc_analyze` tool. These are additive API-client and tool-registration surfaces already covered by the classifier; endpoint documentation lives in `docs/API.md`. No change to the Vale rule set, the `.vale.ini` configuration, the classifier, or `docs/DOC_STYLE.md` style rules.

**2026-05-30 (issue #263 GC-T008 methodology-aware aggregate risk reporting).** Added five new `gc_analyze` kinds (`risk_heatmap`, `risk_distribution`, `risk_top_n`, `risk_trends`, `risk_posture`) registered in `mcp/ground-control/index.js` and backed by the matching helpers (`analyzeRiskHeatmap`, `analyzeRiskDistribution`, `analyzeRiskTopN`, `analyzeRiskTrends`, `analyzeRiskPosture`) in `mcp/ground-control/lib.js`. The Zod shape adds five params (`methodology_profile_id`, `group_by`, `limit`, `order_by`, `from`/`to`/`bucket`) constrained by three new enum allowlists (`RISK_DISTRIBUTION_GROUP_BY`, `RISK_TOP_N_ORDER_BY`, `RISK_TRENDS_BUCKETS`) that mirror the backend `RiskDistributionGroupBy` / `RiskTopNOrderBy` / `RiskTrendsBucket` enums. Each kind dispatches through `request()` to a fixed `/api/v1/analysis/grc/risk-*` path per the ANALYZE_KINDS contract pinned in the cross-cutting decisions. Documentation lives in `docs/API.md` ("Methodology-aware Aggregate Risk Reporting" subsection) and the `gc_analyze` tool description in `mcp/ground-control/index.js`. The DOC_STYLE.md prose for the GC-T014 precedent already covers this pattern; the existing line was extended with the GC-T008 kinds as additional examples. No change to the Vale rule set, `.vale.ini`, the classifier, or the doc-coverage gate machinery.
...
**2026-05-30 (issues #260 / #261 / #262 / #754 GC-T005 / T006 / T007 / T015 risk-governance lifecycle).** Added the `APPETITE_TOLERANCE_KINDS`, `CAMPAIGN_PHASES`, and `KRI_THRESHOLD_BANDS` constant arrays to `mcp/ground-control/lib.js` mirroring the three new Java enums (`AppetiteToleranceKind`, `CampaignPhase`, `KriThresholdBand`) on the new risk-governance lifecycle aggregates. Added matching API-client functions (`createRiskAppetiteProfile` / …, `createRiskAssessmentCampaign` / …, `createKeyRiskIndicator` / …, plus `advanceRiskAssessmentCampaignPhase` and `recordKriMeasurement`). Extended `tools/policy/checks.py::ENUM_CONTRACT_INVENTORY` with three rows so ADR-034's enum-mirror gate covers the new enums. Extended the `gc_risk_governance` Zod shape and handler with `risk_appetite_profile` / `risk_assessment_campaign` / `key_risk_indicator` entities and the new `advance_phase` / `record_measurement` actions. Extended `mcp/ground-control/gc-query.js` `GC_QUERY_PATH_ALLOWLIST` with the three new read-path prefixes (`/api/v1/risk-appetite-profiles`, `/api/v1/risk-assessment-campaigns`, `/api/v1/key-risk-indicators`). Documentation lives in `docs/API.md` (the three new endpoint sections plus the GC-T015 extension on Treatment Plans). These are API-client, MCP-adapter, allowlist, and policy-inventory surfaces; no change to the Vale rule set, the `.vale.ini` configuration, the doc-coverage classifier, or `docs/DOC_STYLE.md` itself.

**2026-05-30 (cluster cycle-2 fixes on GC-T005 / T006 / T007 / T015).** Extended `mcp/ground-control/lib.js::TO_CAMEL` with the 13 new snake_case → camelCase entries (`appetite_statement`, `metric_unit`, `yellow_threshold`, `red_threshold`, `appetite_profile_id`, `scheduled_start`, `scheduled_end`, `scoped_asset_ids`, `approval_metadata`, `measured_at`, `risk_assessment_result_id`, `monitored_risk_factors`, `update_cadence`) so the Jackson bind on the backend DTO does not silently drop them on the wire. Added the `REASSESSMENT_TRIGGER_CATEGORIES` and `REASSESSMENT_TRIGGER_TARGET_TYPES` constant arrays to `mcp/ground-control/lib.js` so the `gc_risk_governance` Zod shape and the frontend `api.ts` mirror the extended Java enums (`ReassessmentTriggerCategory` from 5 → 12 values, `ReassessmentTriggerTargetType` from 7 → 12 values). Extended `tools/policy/checks.py::ENUM_CONTRACT_INVENTORY` with the two new reassessment-trigger rows so ADR-034's enum-mirror gate covers them. These are API-client, MCP-adapter, and policy-inventory surfaces; no change to the Vale rule set, the `.vale.ini` configuration, the doc-coverage classifier, or `docs/DOC_STYLE.md` itself.

**2026-05-30 (issues #736 / #763 GC-I003 / GC-I004 executable evidence + continuous compliance monitoring).** Extended `EVIDENCE_SOURCE_KINDS` in `mcp/ground-control/lib.js` with `CI_PIPELINE_RESULT` and `SECURITY_SCAN_RESULT` (GC-I003 external kinds), added `COMPLIANCE_DRIFT_CATEGORIES` and `COMPLIANCE_DRIFT_SEVERITIES` constants mirroring the new Java enums on the GC-I004 `ComplianceDriftEvent` aggregate, extended the `gc_evidence` adapter to forward `expires_at` / `validity_window_days` and the new source kinds, and added `/api/v1/compliance-drift-events` to `GC_QUERY_PATH_ALLOWLIST` (read-only on the MCP side—no new top-level tool; drift events are detector-emitted). Extended `tools/policy/checks.py::ENUM_CONTRACT_INVENTORY` with four rows (`EvidenceSourceKind`, `EvidenceType`, `ComplianceDriftCategory`, `ComplianceDriftSeverity`) so ADR-034's enum-mirror gate covers them. Documentation lives in `docs/API.md` (`EvidenceArtifactRequest` adds `expiresAt` / `validityWindowDays`; new `/api/v1/compliance-drift-events` surface) and the tool description in `mcp/ground-control/gc-evidence.js`. These are config-parser, policy-inventory, and MCP-adapter surfaces; no change to the Vale rule set, `.vale.ini`, the classifier, or `docs/DOC_STYLE.md`.
**2026-05-30 (cluster fair-analytics GC-T011 / GC-T016 / GC-I017).** Added two new `gc_analyze` kinds (`fair_analysis`, `fair_cam_control_analytics`) registered in `mcp/ground-control/index.js`, with matching helpers `analyzeFairQuantitative` and `analyzeFairCamControlAnalytics` in `mcp/ground-control/lib.js`. Extended `tools/policy/checks.py::ENUM_CONTRACT_INVENTORY` with three new FAIR tag enums (`FairCamControlDomain`, `FairLossForm`, `FairStakeholderKind`) so ADR-034's enum-mirror gate covers them. Documentation lives in `docs/API.md` (`GET /api/v1/analysis/grc/fair-quantitative` and `GET /api/v1/analysis/grc/fair-cam-control-analytics` sections) and the tool descriptions in `mcp/ground-control/index.js`. These are MCP-adapter, config-parser, and policy-inventory surfaces; no change to the Vale rule set, the `tools/install-vale.sh` installer, the `.vale.ini` configuration, or `docs/DOC_STYLE.md`.

**2026-05-31 (SonarCloud S6539 fix: GrcAnalysisService Monster-Class).** Introduced `FairAnalysisOrchestrator` in `backend/.../domain/grcanalysis/service/` to group `FairQuantitativeAnalysisService` and `FairCamControlAnalyticsService` behind a single facade, the same pattern as `RiskAnalysisOrchestrator` for cluster-3. This reduces `GrcAnalysisService`'s class-coupling count from 24 to within the S6539 threshold (≤20). The change is a domain-service refactor with no surface impact—no API endpoints, MCP tools, or policy contracts change; the doc targets for `docs/API.md` and `mcp/ground-control/index.js` were already updated by the cluster fair-analytics amendment above.
