...
**2026-05-30 (issue #744 GC-I002 / GC-I005 / GC-I007 / GC-L011 compliance-framework-mapping aggregate).** Promoted the `AuditLinkTargetType.FRAMEWORK` string path into the first-class `ComplianceFrameworkMapping` aggregate. Added two new `gc_analyze` kinds (`compliance_posture`, `cross_framework_gap`) routed through `mcp/ground-control/lib.js` to fixed `/api/v1/analysis/grc/{compliance-posture,framework-gap}` paths and the matching `analyzeCompliancePosture` / `analyzeCrossFrameworkGap` helpers. Registered the new named MCP tool `gc_compliance_framework_mapping` (`create` / `update` / `delete`; reads route through `gc_query`) and added `/api/v1/compliance-framework-mappings` to the `GC_QUERY_PATH_ALLOWLIST` (mirrored in `mcp/ground-control/README.md` and ADR-035 §allowlist). Extended `tools/policy/checks.py::ENUM_CONTRACT_INVENTORY` with three new compliance enums (`ComplianceFrameworkIdentifier`, `CoverageLevel`, `GapSeverity`) so ADR-034's enum-mirror gate covers them. Documentation lives in `docs/API.md` (`GET /api/v1/analysis/grc/compliance-posture`, `GET /api/v1/analysis/grc/framework-gap`, and the `/compliance-framework-mappings` CRUD table) and the tool description in `mcp/ground-control/index.js`. These are MCP-adapter, REST, policy-inventory, and config-parser surfaces; no change to the Vale rule set, `.vale.ini`, the classifier, or `docs/DOC_STYLE.md`.

**2026-05-30 (issue #744 adversarial-review follow-on).** Hardened the `gc_compliance_framework_mapping` MCP tool and its server-side validation:
- `pick()` in `mcp/ground-control/lib.js` now accepts both the existing array-of-snake_case-keys form and a camelCase-to-snake_case rename-object form. The rename-object form previously threw `keys is not iterable` at runtime on every call to `gc_compliance_framework_mapping action=update` and to `gc_risk_control_mapping action=update`, because `for (const k of keys)` does not iterate plain objects.
- The `gc_compliance_framework_mapping update` body in `mcp/ground-control/index.js` is now built with `pick(args, [...])` using snake_case keys, matching the pattern used by neighboring CRUD tools. The new `TO_CAMEL` entries for `framework_identifier` / `framework_version` / `framework_element` / `coverage_level` ensure the snake→camel conversion in `toCamelCase` rewrites the body to the camelCase shape that `UpdateComplianceFrameworkMappingRequest` expects.
...
**2026-05-31 (SonarCloud S6539 fix: GrcAnalysisService Monster-Class).** Introduced `ComplianceAnalysisOrchestrator` in `backend/.../domain/grcanalysis/service/` to group `CompliancePostureService` and `CrossFrameworkGapService` behind a single facade, the same pattern used by `RiskAnalysisOrchestrator` for cluster-3. This reduces `GrcAnalysisService`'s class-coupling count from 24 to within the S6539 threshold (≤20). The change is a domain-service refactor with no surface impact—no API endpoints, MCP tools, or policy contracts change; the doc targets for `docs/API.md` and `mcp/ground-control/index.js` were already updated by the cluster compliance-framework amendment above.
