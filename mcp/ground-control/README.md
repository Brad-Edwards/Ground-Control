...
| `gc_codex_job` | `action`, bounded `job_id` | Poll the shared review/preflight/mechanical background registry. A terminal `result` is the originating tool's unchanged envelope. Review and preflight jobs support cancellation; mechanical jobs return `job_not_cancellable` until their complete execution path honors abort |
