-- GC-T014 / #721: expand the seeded NIST SP 800-30 Rev. 1 methodology profile
-- with the full Rev. 1 decomposition (threat sources, threat events,
-- vulnerabilities, predisposing conditions, threat-source relevance, multi-
-- dimensional likelihood, impact level, assessment timeframe). seedIfMissing()
-- in MethodologyProfileService only handles the missing-row path; existing
-- rows in production / dev / test require this in-place UPDATE so the schema
-- vocabulary used to validate NIST inputs is consistent with the new analysis
-- endpoint (/api/v1/analysis/grc/nist-sp-800-30) and the NistAssessmentService
-- domain model.

UPDATE methodology_profile
SET
    description = 'NIST SP 800-30 Rev. 1 qualitative risk assessment using the full Rev. 1 decomposition (threat sources, threat events, vulnerabilities, predisposing conditions, likelihoods, impact) with the 5x5 risk matrix (Table I-2).',
    input_schema = '{
  "type": "object",
  ...
  "properties": {
    "threat_source": {
      ...
      "properties": {
        "id": {"type": "string"},
        "name": {"type": "string"},
        "kind": {"type": "string", "enum": ["ADVERSARIAL", "NON_ADVERSARIAL"]}
      }
    },
    "threat_event": {
      "type": "object",
    ...
    "threat_event_kind": {"type": "string", "enum": ["ADVERSARIAL", "NON_ADVERSARIAL"]},
    ...
    "predisposing_conditions": {
      ...
      "items": {
        ...
        "properties": {
          "id": {"type": "string"},
          "description": {"type": "string"},
          "pervasiveness": {"type": "string", "enum": ["VERY_LOW", "LOW", "MODERATE", "HIGH", "VERY_HIGH"]}
        }
      }
    },
    "likelihood_initiation": {"type": "string", "enum": ["VERY_LOW", "LOW", "MODERATE", "HIGH", "VERY_HIGH"]},
    "likelihood_adverse_impact": {"type": "string", "enum": ["VERY_LOW", "LOW", "MODERATE", "HIGH", "VERY_HIGH"]},
    "likelihood_overall": {"type": "string", "enum": ["VERY_LOW", "LOW", "MODERATE", "HIGH", "VERY_HIGH"]},
    "impact_level": {"type": "string", "enum": ["VERY_LOW", "LOW", "MODERATE", "HIGH", "VERY_HIGH"]},
  ...
  "properties": {
    "overall_likelihood": {"type": "string", "enum": ["VERY_LOW", "LOW", "MODERATE", "HIGH", "VERY_HIGH"]},
    "impact_level": {"type": "string", "enum": ["VERY_LOW", "LOW", "MODERATE", "HIGH", "VERY_HIGH"]},
    "risk_level": {"type": "string", "enum": ["VERY_LOW", "LOW", "MODERATE", "HIGH", "VERY_HIGH"]},
    "matrix_cell": {"type": "string", "description": "e.g. L3-I4"},
    "derivation": {"type": "string", "description": "analyst-supplied or derived per Table G-5"}
  },
  "required": ["risk_level"],
  "semantics": {
    "scale": "ordinal",
