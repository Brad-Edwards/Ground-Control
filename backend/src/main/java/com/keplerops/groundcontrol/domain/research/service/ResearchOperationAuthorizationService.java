...
/**
 * GC-RSCH-R005 / GC-RSCH-N005 / GC-RSCH-N006 / GC-RSCH-N014 / ADR-084 —
 ...
 * operation (execution is an adapter/orchestrator concern per ADR-084 §4).
 *
...
public class ResearchOperationAuthorizationService {
     ...
     * deciding actor (an {@code AUTONOMOUS} run cannot self-approve — ADR-084 §3;
    ...
    private void validateRequest(RequestOperationAuthorizationCommand command) {
        ...
        // ADR-084 §1: the record must bind a concrete effect request — the
    ...
    /**
     * {@code allowedTools} is the run's declared tool inventory (ADR-084 §2): a
