...
/**
 * GC-RSCH-N011 / ADR-064 — bounded run-status read snapshot composed entirely
...
public record ResearchRunSnapshot(
    ...
    /** Readiness of one artifact type for the run (ADR-064 §4). */
    ...
    /** A gate still awaiting a required human decision (ADR-064 §3). */
    public record PendingGate(ResearchGatePoint gatePoint, ResearchRunStage guardedStageExit) {}

    /** Bounded source-disposition counts (ADR-064 §5). */
    ...
    /** Budget caps separated from observed usage (ADR-064 §7). */
    public record Cost(
            Long budgetTokens,
    ...
    /** Bounded most-recent failure observation (ADR-064 §6); null when none. */
